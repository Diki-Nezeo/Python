...
# Fungsi untuk mencari ruangan menggunakan Divide and Conquer
def find_room_dnc(rooms, mahasiswa):
    sorted_rooms = sorted(rooms.items(), key=lambda x: x[1])
    return find_room_recursive(sorted_rooms, mahasiswa)

def find_room_recursive(sorted_rooms, mahasiswa):
    if not sorted_rooms:
        return None
    mid = len(sorted_rooms) // 2
    room, capacity = sorted_rooms[mid]
    if capacity >= mahasiswa:
        result = find_room_recursive(sorted_rooms[:mid], mahasiswa)
        return result if result else room
    else:
        return find_room_recursive(sorted_rooms[mid+1:], mahasiswa)
    ...
    for day in available_days:
        for time in available_times:
            if any(s["hari"] == day and s["jam"] == time and s["kelas"] == kelas for s in schedule):
                continue  # Jika kelas yang sama sudah ada di waktu ini, cari waktu lain
            
            available_rooms = [room for room in suitable_rooms if not any(
                s["hari"] == day and s["jam"] == time and s["ruang"] == room for s in schedule)]
            
            if available_rooms:
                selected_room_dnc = find_room_dnc({room: test_rooms[room] for room in available_rooms}, mahasiswa)
                selected_room_greedy = find_room_greedy({room: test_rooms[room] for room in available_rooms}, mahasiswa)
                selected_room = selected_room_dnc or selected_room_greedy
