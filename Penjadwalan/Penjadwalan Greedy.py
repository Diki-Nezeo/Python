...
# Fungsi untuk menambahkan jadwal ujian secara manual
def add_schedule_manual():
    ...
    success = add_schedule(matkul, kelas, semester, mahasiswa)
    if success:
        update_schedule()
...
# Fungsi untuk menambahkan jadwal ujian (manual & dari Excel)
def add_schedule(matkul, kelas, semester, mahasiswa):
...
def load_from_excel():
    ...
    try:
        ...
        for _, row in df.iterrows():
            success = add_schedule(row['Mata Kuliah'], row['Kelas'], row['Semester'], int(row['Jumlah Mahasiswa']))
            if not success:
                messagebox.showwarning("Peringatan", f"Tidak bisa menambahkan jadwal {row['Mata Kuliah']} untuk {row['Kelas']}.")
        update_schedule()
